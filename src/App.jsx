...
function PhoneEntry({ phone, onChange, onRemove, showRemove }) {
  const base = { background: C.navy800, border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, fontSize: 13, outline: "none", padding: "7px 10px" };
  return (
...
function EditableList({ title, color, icon, items, setItems }) {
  ...
  const inputS = { background: C.navy700, border: `1px solid ${C.border}`, borderRadius: 7, padding: "7px 11px", color: C.text, fontSize: 13, outline: "none" };
...
function SettingsSection({ db, setDb }) {
  ...
  const inputBase = { width: "100%", background: C.navy700, border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 12px", color: C.text, fontSize: 14, outline: "none" };
  const SLabel = ({ children }) => <div style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 5 }}>{children}</div>;
